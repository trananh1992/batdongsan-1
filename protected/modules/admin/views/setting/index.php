					...
					if ($itemObject->items && is_array($itemObject->items)):
						...
						<div class="form-group form-group-sm">
							...
							if ($dataObj->controlTyle == 'text'):
								echo '<div class="col-sm-6">' . $form->textField($model, $dataObj->name, array_merge($dataObj->htmlOptions, array('class' => "form-control"))) . $unit . $note . '</div>'; 
							elseif($dataObj->controlTyle == 'textarea'):
								echo '<div class="col-sm-6">' . $form->textArea($model, $dataObj->name, $dataObj->htmlOptions) . $note . '</div>'; 
							elseif($dataObj->controlTyle == 'dropdown'):
								echo '<div class="col-sm-6">' . $form->dropDownList($model, $dataObj->name, $dataObj->data, array('class' => "form-control")) . $unit . $note . '</div>'; 
							elseif($dataObj->controlTyle == 'password'):
								echo '<div class="col-sm-6">' . $form->passwordField($model, $dataObj->name, array_merge($dataObj->htmlOptions, array('class' => "form-control"))) . $unit . $note . '</div>';
